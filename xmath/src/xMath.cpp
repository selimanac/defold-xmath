...
static int xMath_matrix(lua_State *L) {
  ...
  } else if (lua_gettop(L) == 2 && dmScript::IsMatrix4(L, 1) &&
             dmScript::IsMatrix4(L, 2)) {
    Vectormath::Aos::Matrix4 *out = dmScript::CheckMatrix4(L, 1);
...
static int xMath_matrix_frustum(lua_State *L) {
  ...
  if (dmScript::IsMatrix4(L, 1)) {
    ...
    *out = Vectormath::Aos::Matrix4::frustum(left, right, bottom, top, near_z,
                                             far_z);
  }
...
static int xMath_matrix_orthographic(lua_State *L) {
  ...
  if (dmScript::IsMatrix4(L, 1)) {
    ...
    float near_z = (float)luaL_checknumber(L, 6);
    float far_z = (float)luaL_checknumber(L, 7);
    *out = Vectormath::Aos::Matrix4::orthographic(left, right, bottom, top,
                                                  near_z, far_z);
  }
...
static int xMath_quat_matrix4(lua_State *L) {
  ...
  if (dmScript::IsQuat(L, 1)) {
    ...
    Vectormath::Aos::Matrix3 m3(m->getCol0().getXYZ(), m->getCol1().getXYZ(),
                                m->getCol2().getXYZ());
  ...
  if (dmScript::IsMatrix4(L, 1)) {
    ...
    if (nargs == 2) {
      if (dmScript::IsVector3(L, 2)) {
        ...
        *out =
            Vectormath::Aos::Matrix4::scale(Vectormath::Aos::Vector3(s, s, s));
      }
    } else if (nargs == 4) {
      float x = (float)luaL_checknumber(L, 2);
      float y = (float)luaL_checknumber(L, 3);
...
DM_DECLARE_EXTENSION(xMath, LIB_NAME, xMathAppInitialize, xMathAppFinalize,
                     xMathInitialize, 0, 0, xMathFinalize)
